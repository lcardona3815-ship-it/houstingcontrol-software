...
│   └── trazabilidad.md         # historia → tabla → constraint → evidencia (revisión Corte 1)
...
├── scripts/
│   └── verify-reproducibility.sh  # clon limpio -> HU-07 end-to-end (revisión Corte 1)
└── frontend/                   # React (Vite)
    └── src/
```
...
- `tests/pqrs.service.test.js` — la primera prueba TDD del proyecto (RED → GREEN),
  con la estructura PREPARAR/EJECUTAR/VERIFICAR explícita.
- `tests/reservas.concurrencia.test.js` — la evidencia exigida en `docs/problema-duro.md`:
  10 solicitudes concurrentes para el mismo espacio/horario, ejecutado 3 veces, con el
  detalle de cada corrida (operación, recurso, resultado por solicitud, condición de
  éxito) escrito en `backend/tests/evidencia/`.

### Verificación de reproducibilidad

Responde directamente a la pregunta *"clono el repo limpio, ¿levanto todo solo con el
README?"*: reconstruye la base desde cero, levanta el backend y ejecuta HU-07 de punta
a punta contra la API real.

```bash
./scripts/verify-reproducibility.sh
```

Ver `docs/trazabilidad.md` para la cadena completa historia → tabla → constraint →
evidencia, las preguntas de defensa de HU-06 (UNIQUE vs `version`), y la justificación
de cada tabla del esquema frente al modelo de dominio y las historias de usuario.
